...
class EnvWeights:
    def __init__(self, *, override=False,
                 S=0.0, D=0.0, len_diffusivity=0.0, angle_diffusivity=0.0,
                 bias_x=0.0, bias_y=0.0):
        # Enforces valid weight range of zero to one
        if S > 1.0 or D > 1.0 or len_diffusivity > 1.0 or bias_x > 1.0 or bias_y > 1.0 \
                or S < 0.0 or D < 0.0 or len_diffusivity < 0.0 or bias_x < 0.0 or bias_y < 0.0:
            raise ValueError("Weights must be in range [0.0, 1.0]")
        self._profile = EnvWeightProfile(override, S, D, len_diffusivity, angle_diffusivity, bias_x, bias_y)
    ...
    @classmethod
    def full(cls):
        return cls(override=True, S=1.0, D=1.0, len_diffusivity=0.0, angle_diffusivity=0.0, bias_x=1.0, bias_y=1.0)
