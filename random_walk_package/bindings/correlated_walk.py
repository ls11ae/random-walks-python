...
dll.correlated_visit.argtypes = [
    ...
    c_ssize_t,  # start_x
    c_ssize_t,  # start_y
    POINTER(c_bool),  # target_area
    c_bool,  # use_serialization
    c_char_p,  # output_folder
]
dll.correlated_visit.restype = POINTER(POINTER(Tensor))
...
def correlated_walk_visit_probability(kernel, width=100, height=100, time=50, start_x=None, start_y=None, target_area=None, use_serialization=False,
                         output_folder=None):
    if kernel is None:
        raise ValueError("Kernel is None.")
    return dll.correlated_visit(width, height, kernel, time, start_x, start_y, target_area, use_serialization,
                               output_folder.encode('utf-8'))
...
def correlated_utilization_distribution(dp_mat, T, kernels, end_x, end_y, direction=0, use_serialization=False, dp_folder=None, output_folder=None):
    if dp_mat is None and use_serialization is False:
        raise ValueError("DP matrix is None.")
    if kernels is None:
        raise ValueError("Kernels are None.")
