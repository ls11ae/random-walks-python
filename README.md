...
```bash
git clone git@github.com:ls11ae/random-walks-python.git
```

```bash
cd random-walks-python/
```
...
This package provides several random-walk models—including Brownian, correlated, biased, mixed, and time-aware walkers,
implemented in optimized C and optionally speeded up via CUDA.
It is designed for movement simulation, ecological modeling, and research workflows (i.e. MoveApps) involving Movebank
...
There are Jupyter notebooks (one per random walks model) in the ```demos``` folder that demonstrate the use of the
package.
