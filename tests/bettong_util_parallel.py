...
7) Pastes each segment UD into a global (track-level) UD array with correct offsets + clipping (SERIAL aggregation)
8) Builds a home-range mask as the union of per-segment 95% isopleths (SERIAL aggregation)
9) Plots WorldCover background with UD contours and trajectory overlay
...
def plot_worldcover_with_trajectory_and_contour(bettong, raster_tif_mga55,
    ...
    # 95% UD contour for total utilization
    ...
    ax.set_title("Trajectory over ESA WorldCover (MGA55) + UD + Home-range (union) contour")
...
# -------------------------
# FAST WorldCover mass summary (vectorized, no per-cell raster reads)
# Relies on globals set in __main__ before forking.
# -------------------------
G_WC_BAND = None
G_WC_INV_TRANSFORM = None
G_WC_HEIGHT = None
G_WC_WIDTH = None


def segment_worldcover_mass_fast(seg, seg_min_x, seg_min_y):
    """
    Vectorized aggregation of UD mass by WorldCover code using globally loaded raster band.

    seg: 2D numpy array (H, W) weights
    seg_min_x, seg_min_y: map coords for seg's origin such that:
        x = seg_min_x + col
        y = seg_min_y + row
    """
    global G_WC_BAND, G_WC_INV_TRANSFORM, G_WC_HEIGHT, G_WC_WIDTH

    seg = np.asarray(seg, dtype=np.float64)
    H, W = seg.shape
    rows, cols = np.nonzero(seg > 0)
    if rows.size == 0:
        return pd.DataFrame(columns=["wc_code", "wc_name", "mass", "proportion"]), 0.0
    ...
    cc_f, rr_f = G_WC_INV_TRANSFORM * (xs, ys)
    cc = np.floor(cc_f).astype(np.int64)
    rr = np.floor(rr_f).astype(np.int64)

    ok = (rr >= 0) & (rr < G_WC_HEIGHT) & (cc >= 0) & (cc < G_WC_WIDTH)
    if not np.any(ok):
        total_mass = float(np.sum(seg))
        return pd.DataFrame(columns=["wc_code", "wc_name", "mass", "proportion"]), total_mass

    rr = rr[ok]
    cc = cc[ok]
    vals = vals[ok]

    codes = G_WC_BAND[rr, cc].astype(np.int64)

    df = pd.DataFrame({"wc_code": codes, "mass": vals})
    g = df.groupby("wc_code", as_index=False)["mass"].sum()
    total_mass = float(np.sum(seg))
    g["proportion"] = g["mass"] / total_mass if total_mass > 0 else np.nan
    g["wc_name"] = g["wc_code"].map(lambda c: WC_CLASSES.get(int(c), ("Unknown", None))[0])
    g = g.sort_values("mass", ascending=False)
    return g[["wc_code", "wc_name", "mass", "proportion"]], total_mass
...
def compute_one_segment_job(i: int, b_id: str):
    """
    Worker: compute seg UD, local 95% mask, and WorldCover mass table.
    Returns tuple or None if skipped.
    """
    ...
    seg_mask_local, _ = ud_isopleth_mask(seg, p=0.95)

    dx = seg_min_x - G_GLOBAL_MIN_X
    dy = seg_min_y - G_GLOBAL_MIN_Y

    df, total_mass = segment_worldcover_mass_fast(seg, seg_min_x, seg_min_y)
    df2, total_mass2 = segment_worldcover_mass_fast(seg_mask_local.astype(np.float64), seg_min_x, seg_min_y)
    
    df2 = df2.rename(columns={"mass": "mask_mass"})
    df2 = df2.rename(columns={"proportion": "mask_proportion"})
    
    df = df.merge(df2, on=["wc_code", "wc_name"], how="outer")
...
if __name__ == "__main__":
    ...
    print(f"Proccessing {sys.argv[1]}")
    ...
    for bettong_id, bettong in bettongs.items():
    #     print(f"Bettong ID: {bettong_id}, {len(bettong)} fixes")
    
    # exit()
    # Select a short tomato chunk
        # bettong = bettongs["tomato"]
        bettong = bettongs[sys.argv[1]]
        ...
        # Fetch WorldCover for this track
        ...
        # Load WorldCover once in the parent BEFORE forking (shared copy-on-write)
        with rasterio.open(worldcover_mga55_tif) as src:
            wc_band = src.read(1)
            wc_inv = ~src.transform
            wc_h, wc_w = src.height, src.width

        G_WC_BAND = wc_band
        G_WC_INV_TRANSFORM = wc_inv
        G_WC_HEIGHT = wc_h
        G_WC_WIDTH = wc_w
        ...
        # Global arrays (aggregated in main process)
        ...
        with ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as ex:
            futures = [ex.submit(compute_one_segment_job, i, bettong_id) for i in range(n_segments)]
            ...
            for fut in as_completed(futures):
                ...
                # Aggregate serially to avoid race conditions
        ...
        # Home-range (union mask) WorldCover composition (area-based: 1 inside mask)
        df_home, total_mass_home = segment_worldcover_mass_fast(
            home_range_mask.astype(np.float64),
            global_min_x,
            global_min_y
        )
        df_home.insert(0, "bettong_id", bettong_id)
        home_range_dfs.append(df_home)
        
        print("Home-range (union mask) land-cover composition:")
        print(df_home)

        # plot_worldcover_with_trajectory_and_contour(
        #     bettong,
        #     worldcover_mga55_tif,
        #     total_utilization,
        #     home_range_mask,
        #     global_min_x,
        #     global_min_y
    ...
    out_csv = "segment_worldcover_mass_all_bettongs.csv"
    if all_dfs:
    ...
    out_csv = "homerange_mass_all_bettongs.csv"
    if home_range_dfs:
        big = pd.concat(home_range_dfs, ignore_index=True)
        big.to_csv(out_csv, index=False)
        print(f"Wrote {len(big)} rows to {out_csv}")
    else:
        print("No valid segments produced any results; nothing written.")
